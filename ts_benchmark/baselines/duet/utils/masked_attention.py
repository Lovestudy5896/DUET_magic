...
class Mahalanobis_mask(nn.Module):
    ...
    def calculate_prob_distance(self, X):
        XF = torch.abs(torch.fft.rfft(X, dim=-1))# 频域特征
        X1 = XF.unsqueeze(2)
        X2 = XF.unsqueeze(1)
        ...
        temp = torch.einsum("dk,bxck->bxcd", self.A, diff)# 线性变换

        dist = torch.einsum("bxcd,bxcd->bxc", temp, temp)# 距离矩阵

        # exp_dist = torch.exp(-dist)
        exp_dist = 1 / (dist + 1e-10)# 概率转换
        # 对角线置零
    ...
    def forward(self, X):
        ...
        return mask
#通过采样得到二进制掩码 mask，其中 1 表示通道间强相关，0 表示弱相关或无关。